...
src/
  ...
  main.py          パイプライン結合: Channel選択(--channel mic 等) → decode → lookup → display
                   **受信ループ型**（連続待ち受け）。--offline でlookupをローカル辞書に切替（デモ保険）
tests/
...
## Supabase セキュリティ方針
- 受信側で使うのも anon key のみ。service_role key は使わない・置かない
- キーは `.env` に置き gitignore する（リポジトリにコミットしない）
- 受信側は `urls` テーブルの SELECT（逆引き）しか行わない
...
## コミュニケーション
- 進捗報告・説明・コミットメッセージの説明文は日本語で書く
- コード内のコメントも日本語でよい（ただし英語で書いても可。要は「誰が見てもわかる」ことが大事）