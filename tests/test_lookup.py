...
def test_offline_known_ids_return_urls():
    ...
    assert offline_lookup_url(42) == _OFFLINE_URLS[42]
    assert offline_lookup_url(7) == _OFFLINE_URLS[7]
    # デモ保険なので中身が空でない（http で始まる）こと。
    assert offline_lookup_url(42).startswith("http")


def test_offline_unknown_id_returns_none():
    """未登録 id では None を返す（落ちない）。"""
    assert 123 not in _OFFLINE_URLS
